...
3. **Agree, regenerate, commit** — iterate on the `.c4` with the driver; when you both agree it's
   ...
   `pnpm gen:diagrams`) and commit the model to the `feat/<slug>` branch **and push into the
   slice's draft PR** — the step isn't closed until it's on the remote. Never hand-edit a
   generated file.
