...
- **Every step lands pushed, into one draft PR** — the requirements step opens a draft PR for
  the slice; architecture and code push into it; the developer step flips it ready when the runner
  proves the slice. One container, visible from minute one; local-only step results are invisible
  progress.
